...
现在我需要设计一套自动闪避机制，以玩家的触及范围为区域，每tick获取范围内的所有弹射物，对弹射物的动量向量进行预测，如果0.5秒后会击中玩家，就让玩家进行闪避。
静止及走路状态下的单次简单闪避：优先考虑玩家向垂直于弹射物运动方向的方向进行小距离水平闪避（对客户端玩家施加DeltaMovement），取决于弹射物将要命中玩家的左侧还是右侧决定闪避的方向。
闪避失败的情况：例如往右闪避，右边有方块阻挡无法进行闪避；右边脚下没有方块无法着地；往右边闪避会被另一个弹射物击中。
闪避失败的应对：如果会命中玩家的上半身，就趴下后再起身，如果会命中玩家的下半身，就跳起来。
复数弹射物的情况，要找出最优的水平闪避方向，如果没有，再考虑闪避失败的应对。
对于闪避失败的应对，如果预测趴下仍会被击中，就趴下后朝最优的水平闪避方向滑行（触发本模组的slide）；如果预测跳起来后仍会被击中，就跳起来朝最优的水平闪避方向飞扑（触发本模组的leap）。

有问题，第一，现在的闪避是有几率触发，触发几率较低。第二，就算是超平坦世界只有一个弹射物的情况下也会触发趴下，这不对。